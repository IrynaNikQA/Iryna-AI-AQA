...
```bash
npx tsx .cursor/skills/program-deleter/scripts/delete-programs.ts
```

Preview all targets without deleting:

```bash
npx tsx .cursor/skills/program-deleter/scripts/delete-programs.ts --all --dry-run
```

Delete specific program UUID(s) only:

```bash
npx tsx .cursor/skills/program-deleter/scripts/delete-programs.ts --id <PROGRAM_UUID>
```
